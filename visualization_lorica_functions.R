...
##  DEVELOPMENTS
# - when Lorica provides output at t = 0, remove separate reading of dtm_0
# - shiny applications of all plot functions
# - transect: Select soil profiles on the order of the rows/cols inputs, instead of the order of the soils dataframe
...
# Read functions ####
read_lorica_output= function(dtm_0, directory = "", years = NULL, scenario_name = "", 
                             output_rasters = c('water_erosion','tillage', 'dz_soil', "soildepth"))
{
  # scenario name for objects
  if(nchar(scenario_name)>0){scen_add = paste0(scenario_name, "_")} else { scen_add = "" }
  ...
  if(is.null(years))
  {
    years = list.files(path = directory) %>%
      ...
      parse_number() %>%
      sort()
    assign(paste("years",scenario_name, sep = "_"), years, envir = .GlobalEnv) # write to global environment
  }
  
  # Read elevation data
  dtm_stack = c(dtm_0, rast(paste0(directory,"/0_",years,"_out_dtm.asc")))
  names(dtm_stack) = c("dtm_0", paste0("dtm_",years))
  assign(paste0(scen_add, "dtm"), dtm_stack, envir = .GlobalEnv)
  
  # Read other raster data
  for (v in output_rasters)
  {
    if(file.exists(paste0(directory,"/0_",years[1],"_out_",v,".asc")))
    {
      ...
      assign(paste0(scen_add, v), stack, envir = .GlobalEnv)
    }
  }
  ...
  soils = read_csv(paste(directory,filenames, sep = "/")) %>%
    ...
    assign(paste0(scen_add, "soils"), ., envir = .GlobalEnv)
  
  # Read the excel spreadsheet with descriptions of the variables
  if(file.exists("Description Lorica output.xlsx"))
  {
    assign("output_descriptions", read_xlsx("Description Lorica output.xlsx", sheet = 2), 
           envir = .GlobalEnv)
  }
}


depth_plot = function(soils, row_plot, col_plot, t_plot, variablename, title = NULL, ylim = NULL)
{
